...
> 当前版本：`0.1.2 Alpha`。不同账号、课程和平台响应之间仍可能存在兼容性差异。远程请求被接受不代表平台最终统计一定发生变化。
...
- 协调的三栏深色工作台，支持 `80%` 至 `200%` 界面缩放、`Ctrl+滚轮` 和 `Ctrl+0`。
...
- 高对比度单元与小课复选框，支持整行点击、搜索、批量选择和选择数量统计。
- 统一的配置中心、小课选择窗口、运行指标和结构化日志，适配高 DPI 显示。
