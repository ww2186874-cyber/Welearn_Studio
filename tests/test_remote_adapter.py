...
class TimedWorkflowTests(unittest.TestCase):
    def test_full_intervals_heartbeat_and_remainder_wait_then_save(self) -> None:
        waiter = RecordingWaiter()
        transport = RecordingTransport(
            [
                FakeResponse(load_json("sco_state.json")),
                FakeResponse(text="synthetic-ticket-initial"),
    ...
    def test_seconds_below_interval_wait_without_periodic_heartbeat(self) -> None:
        waiter = RecordingWaiter()
        transport = RecordingTransport(
            [
                FakeResponse(load_json("sco_state.json")),
                FakeResponse(text="synthetic-ticket"),
                FakeResponse({"ret": 0}),
            ]
        )
    ...
    def test_cancellation_during_wait_skips_heartbeat_and_save(self) -> None:
        ...
        cancellation = StubCancellation()
        transport = RecordingTransport(
            [FakeResponse(load_json("sco_state.json")), FakeResponse(text="synthetic-ticket")]
        )
        result = WeLearnRemoteClient(transport, waiter=waiter).run_timed_study(
    ...
    def test_malformed_state_and_missing_save_ret_remain_unknown(self) -> None:
        malformed_transport = RecordingTransport([FakeResponse({"comment": {"cmi": {}}})])
